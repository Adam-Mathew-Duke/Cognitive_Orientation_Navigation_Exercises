    ...
    _initListeners() {
        this.tool.onMouseDown = (event) => {
            ...
            if (hitResult && hitResult.item) {
                ...
                this.draggedItem = target.data && target.data.isCap ? target : (target.parent && target.parent.data && target.parent.data.isCap ? target.parent : null);
            } else {
                this.draggedItem = null;
            }
        };
        ...
        this.tool.onMouseUp = (event) => {
            if (this.draggedItem) {
                ...
                var dome = new window.paper.Path.Circle({ center: [0, 0], radius: 11, fillColor: '#ff04fb', strokeColor: '#e20cb7', strokeWidth: 1.5 });
                var brim = new window.paper.Path.Arc({ from: [-12, 3], through: [0, 9], to: [12, 3], strokeColor: '#e20cb7', strokeWidth: 2.5 });
                var group = new window.paper.Group([dome, brim]);
                group.position = snappedPoint;
                group.data.isCap = true;
                this.saveState();
            }
            this.downPoint = null;
        };
